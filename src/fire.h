...
public:
    ...
    ClassicFireEffect(int size, int offset, int cooling = 80, int sparking = 50, int sparks = 3, int sparkHeight = 4, bool breversed = true, bool bmirrored = true) 
        : Size(size),
          Offset(offset),
          Cooling(cooling),
          Sparks(sparks),
    ...
    virtual void DrawFire()
    {
        ...
        for (int i = 0; i < Size; i++){
            //Serial.printf("heat[%2d] %3d -> ",i, heat[i]);
        ...
        for (int i = 0; i < Sparks; i++)
        {
            if (random(255) < Sparking)
            {
                ...
                //Serial.printf("heat[%2d] %3d -> ", y, heat[y]);
        ...
        for (int i = 0; i < Size; i++)
        {
            ...
            DrawPixels(j + Offset, 1, color);
            if (bMirrored)
            {
                int j2 = !bReversed ? (2 * Size - 1 - i) : Size + i;
                DrawPixels(j2 + Offset, 1, color);
            }
        }
    }
};
